...
class DisciplinaDAO
{
    ...
    public function getDisciplinaWithAlunos($disciplinaID)
    {
        ...
        */
        ...
        $sql = 
        "SELECT d.id, d.nome, a.id AS aluno_id, a.nome AS aluno_nome
        FROM disciplina d
        JOIN disciplina_aluno da ON d.id = da.disciplina_id
        WHERE da.aluno_id = :disciplinaID";
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $disciplinas[] = [ 
                'disciplina' => new Disciplina($row['disciplina_id'], $row['nome']), 
                'alunos' => $row['aluno_nome']
            ];
    ...
    {
        $sql = 
        "SELECT p.id, p.nome, p.disciplina_id 
        FROM professor p
